...
4. **RBAC/namespace content is orphaned from automation** (see above) — real on both clusters, but not currently reconciled by any Policy or Application. Next step is extending the pull-model pattern to manage it, the same way `capdev-business-apps`'s `nginx-demo` is already managed as a real onboarded workload.
